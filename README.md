...
### CMU 11-411/11-611 Natural Language Processing, 2023f (David R. Mortensen)
* Changdae Oh, Sangyoon Lee, Junyong Lee, Yunjin Na, Jooeon Kang

## Project Overview
> To address quadratic computational complexity issues of Transformer-based Large Language Models (LLMs), RNN-based alternative have recently been proposed. In this project, we reproduce the empirical results from one of the most representative RNN-based LLM, RWKV, and further investigate how the prompt engineering effect on RNN-based LLM and how different the effect from that on the Transformer-based LLM.

## Methodology
* We bollow most of the experiment pipelines from [lm-evaluation-harness](https://github.com/EleutherAI/lm-evaluation-harness) library and adapt some modules to enable comprehensive analysis on the prompt engineering.
* First, we add an attribute `self.prompt_type` in the ancestor class of downstream task (please refer the line 516 of `lm_eval/base.py` module), and in the `main.py`, we add `prompt_type` argument, which is modified to validate each corresponding prompt design.
* Second, we wrote the natural language prompt templates that we denote in our report (Figure 1) to individual tasks' modules, i.e., `lm_eval/tasks/piqa.py`, `lm_eval/tasks/arc.py`, and `lm_eval/tasks/sciq.py` modules.
* After modifying the input arguments of `simple_evalute()` function of `lm_eval/evaluator.py` module to receive prompt_type argument, we wrote the code for specifying prompt types for downstream tasks in line 111 of that `lm_eval/evaluator.py` module.
* By doing so, test dataset will be processed through the user-specified prompt template.
* For all experiments (four LMs and five tasks), we follow the instruction and default hyperparameter settings of this [lm-evaluation-harness](https://github.com/EleutherAI/lm-evaluation-harness) library.

---

<br>

below, we attach the README.md of the original lm-evaluation-harness repository:
